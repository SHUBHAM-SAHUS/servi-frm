...
class EmailSWMSSign extends React.Component {
  blobData = ''
  constructor(props) {
    super(props);
    this.state = {
    ...
    // this.props.change('from_field', "prasadd@tangible.tech");
    ...
    this.props.change('from_profile', this.state.adminDetails.role && this.state.adminDetails.role.role_name);
    this.props.change('to_field', this.props.job.quote && this.props.job.quote.client && this.props.job.quote.client.contact_person_email);
  ...
  editorState = () => {
    ...
    if (body.includes("{{link}}")) {
      body = body.replace("{{link}}", `<a href='${window.location.origin + '/sign_swms_preview?job_number=' + this.props.job.job_number}' target='_blank'>click here</a>`)
    }

    const html = body
    //             ?
    //             this.props.emailTemplateList.body
    //             :
    //             `<p>Hello ${this.props.job && this.props.job.quote && this.props.job.quote.client && this.props.job.quote.client.name},<br><br>
    //     Please find attached the SWMS sign off Report of job #${this.props.job.job_number} aginst quote #${this.props.job.quote.quote_number}.
    // You can also <a href='${window.location.origin + '/sign_swms_preview?job_number=' + this.props.job.job_number}' target='_blank'>click here</a> to view the job report.
  ...
  componentDidMount(props) {
    const slug = 'sign_swms' // sending static value for slug
  ...
  printDocument() {
    const fileUrl = this.props.swmsSignDetails && this.props.swmsSignDetails.job_swms_sign_pdf;
      ...
      fetch(fileUrl)
        .then(response => {
          response.blob()
            .then(blob => {
              this.blobData = blob;
              stopSipnner(this.props.dispatch);
            })
        });
    } else {
      const { job, swmsSignDetails, taskSWMS, swmsDoc, location } = this.props;
      var obj = pdf(<SignSWMSPdf jobDetails={job} swmsSignDetails={swmsSignDetails} taskSWMS={taskSWMS} swmsDoc={swmsDoc} allStaffList={location.state} />).toBlob();
      obj.then((blob) => {
        // console.log(blob)
  ...
  onSubmit = async formData => {
    ...
    // console.log('job report', formData)
            ...
            <div className="sf-card-wrap">
              <div className="sf-card mt-4">
                <div className="sf-card-head abb-1 d-flex justify-content-between align-items-center">
                  <h2 className="sf-pg-heading">{"Send SWMS"}</h2>
...
const mapStateToProps = (state, ownProps) => {
  ...
  return {
    ...
    // initialValues: ({ subject: `SWMS Sign off report for job #${"JSK1001909190001"}` }),
...
export default compose(
  ...
  reduxForm({
    form: 'EmailSWMSSign', validate: JobDocsEmailvalidate, enableReinitialize: true,
