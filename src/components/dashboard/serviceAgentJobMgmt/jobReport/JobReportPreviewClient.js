...
import { Icon, Collapse, Table, Modal, notification } from 'antd';
...
class JobReportPreviewClient extends React.Component {
    ...
    componentDidMount() {
        ...
        if (query.get('job_number')) {
            this.props.action.getJobReport(null, query.get('job_number'));
                ...
                <div className="sf-card-wrap">
                    ...
                    <div className="jdp-head">
                        ...
                        {job.quote && job.org_details && job.org_details.client_logo ?
                            ...
                            <strong className="img-abbri-str">{job.org_details && job.org_details.name &&
                                abbrivationStr(job.org_details.name)}</strong>}
                    </div>
                    {/* inner header */}
...
const mapStateToProps = (state) => {
    return {
        ...
        job:
            state.sAJobMgmt.jobDetails && state.sAJobMgmt.jobDetails.job_details && state.sAJobMgmt.jobDetails.job_details[0] ?
...
export default connect(mapStateToProps, mapDispatchToprops)(JobReportPreviewClient);