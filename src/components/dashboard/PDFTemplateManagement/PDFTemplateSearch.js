...
class PDFTemplateSearch extends React.Component {
    constructor(props) {
        super(props);
    ...
    componentWillReceiveProps(props) {
        if (!props.location.pathname.includes("show_pdf_template"))
    ...
    render() {
        const dataSource = this.props.pdfTemplateList && this.props.pdfTemplateList.filter(pdf => pdf.template_name.toLowerCase().includes(this.state.seacrhValue.toLowerCase()));
        return (
                ...
                <div className="sf-card">
                    <div className="sf-card-head">
                        ...
                        <div className="auto-search-txt search-lists">
                            <AutoComplete
                                ...
                                size="large"
                                style={{ width: '100%' }}
                                // dataSource={this.state.viewButton ? dataSource.map(pdf => pdf.template_name).slice(0, 5) : dataSource.map(pdf => pdf.template_name)}
                    ...
                    <div className="sf-search-body">
                                ...
                                <List className="sf-service-list"
                                    ...
                                    renderItem={item => (
                                        <List.Item className={this.state.activeId != null ?
                                            ...
                                            <Link to={{ pathname: this.props.match.path + '/show_pdf_template', state: item.slug }}>
                                                <List.Item.Meta
                                                    avatar={<label>{abbrivationStr(item.template_name)}</label>}
                        ...
                        {dataSource.length >= 10 ? <div className="sf-card-footer d-flex align-items-end justify-content-center" >
                            <button className="v-all-org" style={{ display: this.state.viewButton ? 'block' : 'none' }} onClick={this.handelViewAllClick}>{Strings.view_all_pdf}</button>
                        </div> : null}
                    </div>
                </div>
            </div>
...
export default withRouter(connect(mapStateToProps, actions)(PDFTemplateSearch))