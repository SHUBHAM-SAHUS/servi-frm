...
export class ServiceAgentCalendar extends Component {

  constructor(props) {
    super(props);

    if (JSON.parse(getStorage(ADMIN_DETAILS)).organisation.organisation_type == 1) {
  ...
  handleChangeView = view => {
    this.setState({ showJobDetails: view ? true : false })
  }
            ...
            <div className="row">
              <div className={this.state.showAcceptedJobs ? "col-md-9 flex-wrap d-flex justify-content-between calender-heading" : "col-md-12 flex-wrap d-flex justify-content-between calender-heading"}>
                <div className="status-bnt">
                  <button type="button" onClick={() => this.handleNewClick()} className={this.state.new ? `not-approved-active` : `not-approved`}>Job Offer</button>
                  <button type="button" onClick={() => this.handleAcceptedClick()} className={this.state.accepted ? `approved-active` : `approved`}>Job Accepted</button>
                  <button type="button" onClick={() => this.handleStartedClick()} className={this.state.started ? `started-active` : `started`}>Job Started</button>
                  <button type="button" onClick={() => this.handlePausedClick()} className={this.state.paused ? `paused-active` : `paused`}>Job Paused</button>
                  <button type="button" onClick={() => this.handleCompletedClick()} className={this.state.completed ? `completed-active` : `completed`}>Job Completed</button>
                  <button type="button" onClick={() => this.handleSignedOffClick()} className={this.state.signedOff ? `signed-off-active` : `signed-off`}>Job Signed Off</button>
                  <button type="button" onClick={() => this.handleInvoicedClick()} className={this.state.invoiced ? `invoiced-active` : `invoiced`}>Invoiced Org</button>
                </div>
...
const mapStateToProps = state => {
  const { selectedJobDetails, currentCalendarEvent, numberOfShifts, jobNotes } = state.sAJobCalendar
  var values = {}
  ...
  values.job_note = jobNotes && jobNotes.length ? jobNotes[0] : null;
  ...
  return {
    ...
    formValues: state.form.serviceAgentCalendar && state.form.serviceAgentCalendar.values && state.form.serviceAgentCalendar.values,
...
export default compose(
  ...
  reduxForm({
    form: 'serviceAgentCalendar',
    onSubmitFail: (errors, dispatch, sub, props) => {
      handleFocus(errors, "#");
    }
  })
)(ServiceAgentCalendar)
