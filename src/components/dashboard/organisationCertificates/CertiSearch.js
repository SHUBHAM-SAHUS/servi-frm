...
class CertiSearch extends React.Component {
  constructor(props) {
    super(props);
    this.state = {
      seacrhValue: '',
  ...
  componentWillReceiveProps(props) {
    if (!props.location.pathname.includes("showCerti"))
  ...
  handleSearchChange = (searchValue) => {
    ...
    if (searchValue.length >= 3) {
      this.props.action.searchExpandOrgCertifcateList(searchValue, 1, true)
        .then(listLength => {
          if (listLength < 10) {
            this.setState({ viewButton: false })
          } else {
      ...
      this.props.action.initOrgCerti()
        .then((flag) => {
          this.setState({ viewButton: true, allLoaded: false })
        })
        .catch((message) => {
  ...
  handelViewAllClick = () => {
    this.props.action.searchExpandOrgCertifcateList(this.state.searchValue, this.props.pageNumber + 1, false)
      .then((listLength) => {
        this.setState({ viewButton: false }, () => {
          if (listLength < 10) {
  ...
  handleScroll = (value) => {
    ...
    if (!this.state.allLoaded && value.topPosition && ((value.containerHeight + value.topPosition) >= value.realHeight)) {
      this.props.action.searchExpandOrgCertifcateList(this.state.searchValue, this.props.pageNumber + 1, false, true)
        .then((listLength) => {
          if (listLength < 10)
            this.setState({ allLoaded: true })
        })
                ...
                <List className="sf-service-list"
                  ...
                  renderItem={item => (
                    <List.Item className={this.state.activeId != null ?
                      ...
                      <Link to={{ pathname: this.props.match.path + '/showCerti', state: item.id }}>
                        <List.Item.Meta
...

            {
              this.state.viewButton && this.props.roles.length >= 10
                ? <div className="sf-card-footer d-flex align-items-end justify-content-center">
                  <button className="v-all-org" onClick={this.handelViewAllClick}>{Strings.view_all_certi_txt}</button>
                </div>
                : null
            }
...
const mapStateToProps = (state) => {
  return {
    roles: state.orgCerti.orgCerti,
    pageNumber: state.orgCerti.currentPageNumber,
...
export default withRouter(connect(mapStateToProps, mapDispatchToprops)(CertiSearch))