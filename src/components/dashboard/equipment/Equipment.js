...
import * as industryManagementActions from '../../../actions/industryManagementAction.js'
...
import AdvanceSearchEquipment from './AdvanceSearchEquipment';

const mapRouteToTitle = {
  '/dashboard/equipment/createEquipment': Strings.equipment
}
...
class Equipment extends React.Component {
  ...
  componentDidMount() {
    ...
    this.props.equipmentActions.getAllTestAndTagType()
      .then(() => {
        // 
      })
      .catch((message) => {
    ...
    // this.props.equipmentActions.getAllEquipments()
    //   .then(() => {
    //     // 
    //   })
    //   .catch((message) => {
    ...
    this.props.equipmentActions.getTestersList()
      .then(() => {
        // 
      })
      .catch((message) => {
  ...
  createRoleHandler = () => {
    this.props.history.push(this.props.match.path + '/createEquipment')
  }
  ...
  componentWillReceiveProps(props) {
    ...
    if (this.props.location.pathname.includes("/createEquipment")) {
    ...
    } else if (this.props.location.pathname.includes("/showEquipment")) {
      ...
      <div className="sf-page-layout">
        ...
        <div className="dash-header">
          <h2 className="page-mn-hd">
            ...
            {mapRouteToTitle[this.props.location.pathname] ? mapRouteToTitle[this.props.location.pathname] : Strings.equipment}
          </h2>
          ...
          <div class="oh-cont">
            ...
            <button className="bnt bnt-active" onClick={this.createRoleHandler}>{Strings.create_new_equipment}</button>

          </div>
        </div>
        {/* inner header  */}
        <div className="main-container">
          {this.state.advanceSearch ? <AdvanceSearchEquipment setViewButton={this.setViewButton}
          /> : null}
          <div className="row">
            {/* Left section */}
            <EquipmentSearch
              ...
              toggleAdvanceSeacrh={this.toggleAdvanceSeacrhEquipment}
              setViewButton={this.setViewButton}
              viewButton={this.state.viewButton}
            />
            {/* center section  */}
            <Route path={this.props.match.path + '/showEquipment'} render={(props) => <EditEquipment {...props} togleSearch={this.state.togleSearch} />} />
            <Route path={this.props.match.path + '/createEquipment'} render={(props) => <AddNewEquipment {...props} togleSearch={this.state.togleSearch} />} />
          </div>
        </div>
      </div>
    );
...
const mapStateToProps = (state) => {
  return {
    ...
    formValues: state.form.AddNewEquipment,
    formSyncErrors: getFormSyncErrors('AddNewEquipment')(state),
...
const mapDispatchToprops = dispatch => {
  return {
    ...
    industryActions: bindActionCreators(industryManagementActions, dispatch),
  }
}
